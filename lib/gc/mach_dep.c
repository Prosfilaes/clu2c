/* This file has been modified by Department of Mathematical and
   ...
   $Id: mach_dep.c,v 2.7 1996/05/12 07:21:45 ushijima Exp $ */
...
#       endif
#       if defined(M68K) && (defined(SUNOS4) || defined(NEXT) || defined(NEWSOS4) && defined(WITH_GNU_AS))
...
#       if defined(M68K) && defined(NEWSOS4) && !defined(WITH_GNU_AS)
	  /* M68K/NEWSOS.  The following code is identical to one for
             M68K/SUNOS, except that instructions are written in a
             slighly defferent form. */
	
	  asm("subq.w #0x4,sp");	/* allocate word on top of stack */

	  asm("move.l a2,(sp)");	asm("jbsr _GC_push_one");
	  asm("move.l a3,(sp)");	asm("jbsr _GC_push_one");
	  asm("move.l a4,(sp)");	asm("jbsr _GC_push_one");
	  asm("move.l a5,(sp)");	asm("jbsr _GC_push_one");
	  /* Skip frame pointer and stack pointer */
	  asm("move.l d1,(sp)");	asm("jbsr _GC_push_one");
	  asm("move.l d2,(sp)");	asm("jbsr _GC_push_one");
	  asm("move.l d3,(sp)");	asm("jbsr _GC_push_one");
	  asm("move.l d4,(sp)");	asm("jbsr _GC_push_one");
	  asm("move.l d5,(sp)");	asm("jbsr _GC_push_one");
	  asm("move.l d6,(sp)");	asm("jbsr _GC_push_one");
	  asm("move.l d7,(sp)");	asm("jbsr _GC_push_one");
